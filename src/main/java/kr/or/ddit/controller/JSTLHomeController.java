...
public class JSTLHomeController {
	 ...
	 * 이런 중복되는 노력을 없애기 위해서 나오는 것이 JSTL이다
	 * 
	 * 1) core 태그 라이브러리
	 * 
	 * 		요소			|			설명
	 ...
	 * ----------------------------------------------------------------------
	 * 	<c:when>		|	여러 조건을 지정하고 지정한 조건과 일치하는 처리 내용을 구현한다. <c:choose>
	 * ----------------------------------------------------------------------
	 * 	<c:otherwise>	|	<c:when> 요소에서 지정한 조건에 모두 일치하지 않을 때 처리할 내용을 구현한다.<c:choose>
	 * ----------------------------------------------------------------------
	 * 	<c:foreach>		|	 컬렉션이나 배열의 각 항목을 처리할 때 사용한다.
	 * ----------------------------------------------------------------------
	 * 	<c:forTokens>	|	구분자로 구분된 각가의 토큰을 처리할 때 사용한다.	
	 * ----------------------------------------------------------------------
	 ...
	 * ----------------------------------------------------------------------------------	
	 * 	<fmt:formatDate>	|		Data 객체를 문자열로 변환한다.
	 * ----------------------------------------------------------------------------------	
	 * 	<fmt:parseDate>		|			문자열을 Data 객체로 변환한다.
	 * ----------------------------------------------------------------------------------	
	 * ::: taglib 지시자 사용 모습 <%@ taglib uri = "http://java.sun.com/jsp/jstl/fmt" preifx = "fmt" %>
	 * 
	 * 
	 * 3) function 태그 라이브러리
	 * 
	 * 		요소					|			설명
	 * -----------------------------------------------------------------------------------
	 * <fn:contains>			|	지정한 문자열이 포함되어 있는지 확인한다.
	 * -----------------------------------------------------------------------------------
	 * <fn:containsIgnoreCase>	|	지정한 문자열이 대문자/ 소문자 구분하지 않고 포함되어 있는지 판단한다.
	 * -----------------------------------------------------------------------------------
	 * <fn:startsWith>			|	지정한 문자열로 시작하는지 판단한다.		
	 * -----------------------------------------------------------------------------------
	 * <fn:endsWith>			|	지정한 문자열로 끝나는지 판단한다.		
	 * -----------------------------------------------------------------------------------
	 * <fn:indexOf>				|	지정한 문자열이 처음으로 나왔을 떄의 인덱스를 구한다.		
	 * -----------------------------------------------------------------------------------
	 ...
	 * -----------------------------------------------------------------------------------
	 * <fn:toLowerCase>			|	문자열을 소문자로 치환한다.		
	 * -----------------------------------------------------------------------------------
	 * <fn:toUpperCase>			|	문자열을 대문자로 치환한다.		
	 * ----------------------------------------------------------------------------------------
	 ...
	 * 
	 * 6. 코어태그
	 * - 조건 분기나 반복처리 그리고 변수의 지정 등과 같이 논리적인 처리를 위해 사용되는 스크립트 코드를 대체하기 위한 태그를 제공한다.
	 * 
	 * 	1) <c:out> 태그
	 ...
	 * ---------------------------------------------
	 * 		value	|	object		|	 출력할 값	
	 * 		escapeXMl	|	boolean	|	특수문자를 변환할지의 여부
	 * 		default	|	Objecet		|	value의 결과값이 null인 경우 출력할 값
	 * ----------------------------------------------	
	 * 
	 * 
	 * 	2) <c:set> 태그
	 ...
	 * ---------------------------------------------
	 * 	var		|	String		|	EL 변수 이름	
	 * 	value	|	Objecet		|	변수에 할당할 값
	 * 	scope	|	String		|	변수를 생성할 영역, 기본값은 page
	 * 	target	|	Objecet		|	프로퍼티 값을 설정할 객체 지정
	 * 	property|	object		|	프로퍼티 이름
	 * ----------------------------------------------	
	 * 
	 * 
	 * 3) <c:remove> 태그
	 ...
	 * ---------------------------------------------
	 * 	test		|	boolean		|	검사 조건
	 * 	var			|	String		|	검사 조건의 계산 결과 값을 저장할 EL 변수
	 ...
	 * 
	 * 8) <c:otherwise> 태그
	 * 	<c:when> 요소에서 지정한 조건에 모두 일치하지 않을 떄 처리할 내용을 구현한다.
	 * 	<c:choose> 요소에서 사용한다.
	 * 
	 * 9) <c:forEach> 태그
	 ...
	 * 	varStatus	|	String		| 루프 상태를 저장할 EL 변수이름
	 ...
	 * - 절대 URL : 완전한 URL 입니다.
	 * - 상대 URL : 
	 *		> 웹 애플리케이션 내서의 절대 경로 : 슬래시로 시작한다.
	 *		> 현재 JSP에 대한 상대 경로	: 슬래시로 시작하지 않는다.
	 *
	 * 12) <c:url> 태그
	 * - url을 재작성한다.
	 * 
	...
	// 2) escapeXml 속성의 기본값은 true 이므로 특수문자를 변환한다.
	@GetMapping(value="/home0102")
	public String home0102(Model model) {
		Member member= new Member();
		member.setUserId("<p>hongkd0101<>&%</p>");
		model.addAttribute("member",member);
		return "home/jstl/home0102";
	}
	
	// 3) escapeXml 속성 값을 false로 지정하면 특수문자를 변환하지 않는다.
	...
	// 4) value 속셍에 지정한 값이 존재하지 않으면 default 속성에 지정한 값이 출력된다.
	@GetMapping(value ="/home0104")
	...
	public String home0202(Model model) {
		Member member = new Member();
		member.setUserId("hong2");
		model.addAttribute("member",member);
		return "home/jstl/home0202";
	}
		
	// 7) 태그의 몸체값을 넣어 사용
	@GetMapping(value="/home0203")
	public String home0203(Model model) {
		Member member = new Member();
		member.setUserId("hong3");
		model.addAttribute("member",member);
	...
	public String home0301(Model model) {
		Member member = new Member();
		member.setUserId("hong0301");
		model.addAttribute("member",member);
	return "home/jstl/home0301";
	}
	
	// 10) EL식 내부에 발생하는 예외는 EL식 내부에서 처리하므로 catch 변수에 'ex'에 저장되지 않는다.
	...
	// 10) jsp 표현식에서 발생한 예외는 catch 변수 'ex'에 저장된다.
	@GetMapping(value="/home0402")
	public String home0402(Model model) {
		return "home/jstl/home0402";
	}
	
	// 11) test 속성에 true나 false를 리ㅌ너하느 조건문이 온다.
	@GetMapping(value="/home0501")
	...
	// 12) 모든  c:when 태그의 결과가 false이면  otherwise 실행
	@GetMapping(value="/home0601")
	...
	// var 속성에 명시된 변수에 나누어진 문자열을 지정한다.
	@GetMapping(value="/home0801")
